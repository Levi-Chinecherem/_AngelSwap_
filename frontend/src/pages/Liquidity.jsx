...
export default function Liquidity() {
  ...
  useEffect(() => {
    const fetchInitialData = async () => {
      ...
      try {
        ...
        const [tokenList, balances, poolData] = await Promise.all([
        ...
        console.log("Pool initialized status:", initialized);
  ...
  const getBalance = (tokenAddress) => tokenBalances[tokenAddress] ? ethers.formatEther(tokenBalances[tokenAddress]) : "0";
  ...
  const canAddLiquidity = () => {
    const result = (
      ...
      walletAddress && isInitialized && !isAddingLiquidity && !isFetching
    );
    console.log("canAddLiquidity check:", {
      fromAmount,
      toAmount,
      amount1Valid: parseFloat(fromAmount) > 0,
      amount2Valid: parseFloat(toAmount) > 0,
  ...
  const handleAddLiquidity = async () => {
    if (!canAddLiquidity()) {
      toast({ title: "Invalid Input", description: "Enter valid amounts and ensure pool is initialized", variant: "destructive" });
      return;
    }
